...
app = Flask(__name__)
app.secret_key = "ravi-medical-assistant-secret"   # session ke liye zaroori
...
def index():
    ...
    if request.method == "POST":
        ...
        if symptoms_text:
            ...
            precautions_list = list(pre[0]) if len(pre) > 0 else []
            medications_list = list(med) if len(med) > 0 else []
            diets_result = list(die) if len(die) > 0 else []
            workouts_result = list(wrkout) if len(wrkout) > 0 else []
    ...
    # last user input for textarea display
    last_symptoms_text = ""
    for msg in reversed(chat_history):
        if msg["type"] == "user":
            last_symptoms_text = msg["text"]
            break

    return render_template(
        "index.html",
        chat_history=chat_history,
        last_symptoms_text=last_symptoms_text,
        symptom_names=SYMPTOM_NAMES,     # ⭐ VERY IMPORTANT
    )
